...
function CopyLocal{
    ...
    $TargetFolder = [Environment]::GetFolderPath([Environment+SpecialFolder]::CommonApplicationData) + "\\ForensiT\\Scripting"
    ...
    # Provisioning Package File
    $Value = Get-XMLAttributeValue $Config "ProvisioningPackage"

    If(-Not [string]::IsNullOrEmpty($Value)){

        $LookFile = Split-Path $Value -leaf
        $Config = Set-XMLAttributeValue $Config "ProvisioningPackage" $LookFile
        Copy-Item $Value "$TargetFolder\\$LookFile"
    }
